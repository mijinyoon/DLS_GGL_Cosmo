...
ell_input = np.arange(10,2001)
ell_output = np.zeros(len(ell_input))
result = np.zeros(len(ell_input))

for i in range(len(ell_input)):
    ell_output[i],result[i] = P_gm_cal(ell_input[i])

np.savetxt("P_ell_gm.txt", np.vstack((ell_output,result)).T, fmt = '%d  %E')
